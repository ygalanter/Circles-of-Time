...
// based on "minute_to_formal_words" but combines text into single line
void minute_to_common_words(int minutes, char *words) {
  // PBL_ASSERT(minutes >= 0 && minutes < 60, "Invalid number of minutes");

  strcpy(words, "");

  if (minutes == 0) {
    strcat(words, STR_OH_TICK);
    strcat(words, STR_CLOCK);
    return;
  }
  if (minutes < 10) {
    strcat(words, ONES[minutes%10]);
    return;
  }
  if (minutes > 10 && minutes < 20) {
...
//     strcat(words, TEENS_SPLIT[(minutes - 10) % 10][1]);
  ...
  strcat(words, TENS[minutes / 10 % 10]);
